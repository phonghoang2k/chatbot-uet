    ...
    waiting.list((data) => {
        for (let i = 0; i <= data.length; i++) {
            if (i === data.length) {
                ...
                if (mygender == "None")
                    facebook.sendTextMessage(id, language.START_WARN_GENDER);
            ...
            let isGenderMatched =
                (mygender == "None" && target_gender == "None") ||
                (mygender == "Male" && target_gender == "Female") ||
                (mygender == "Female" && target_gender == "Male");
            if (
                data.length > config.MAX_PEOPLE_WAITROOM ||
                ((mygender == "None" || target_gender == "None") &&
                    Math.random() > 0.8)
            ) {
        ...
        if (data.attachments[0]) {
            ...
            if (type === "fallback") {
                if (data.text) messageData.text = data.text;
                    ...
                    messageData.text =
                        language.ATTACHMENT_LINK + data.attachments[0].url;
            } else if (
                !data.attachments[0].payload ||
            ...
            couple.findPartner(sender, (sender2) => {
                ...
                if (!waitstate && sender2 === null) {
                    ...
                    if (command === language.KEYWORD_START) {
                        ...
                        command.startsWith(language.KEYWORD_GENDERPREFER)
                    ) {
                        user.setPreferedGender(
                            sender,
                            command,
                            (id, message) => {
                                switch (message) {
                                    ...
                                    default: {
                                        facebook.sendTextMessage(
                                            id,
                                            `${language.GENDER_WRITE_OK} ${language.GENDER_MAP[message]} ${language.GENDER_WRITE_WARN}`
                                        );
                                        findPair(id, message);
                                    }
                                }
